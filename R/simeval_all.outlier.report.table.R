#' @export
all_outlier_report_table <- function(ofv_outliers,ebe.npde_outliers,cwres.iwres_outliers,residual_names,ID_deleted_ebe=NULL,ID_deleted_ofv=NULL) {
  ...
  if(ncol(all_outlier_table)>0) {
    ...
    all_outlier_table <- all_outlier_table %>%
      dplyr::mutate_at("ofv",dplyr::funs(ifelse(ID %in% ID_deleted_ofv,"NA",.))) %>%
      dplyr::mutate_at("ebe_npde",dplyr::funs(ifelse(ID %in% ID_deleted_ebe,"NA",.))) %>%
