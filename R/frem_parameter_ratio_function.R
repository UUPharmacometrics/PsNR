    ...
    for (j in 1:length(parameter_names)) {
      for (i in 1:length(covariate)) {
        if (covdata$is.categorical[i] != "1") {
          part_5th <- inTable_frem[ , paste0("RATIO.par.",parameter_names[j],".given.cov5th.",covariate[i]) ]
          name_5th <- paste0(covariate[i],".cov5th")
          part_95th <- inTable_frem[ , paste0("RATIO.par.",parameter_names[j],".given.cov95th.",covariate[i]) ]
          ...
          part <- inTable_frem[ , paste0("RATIO.par.",parameter_names[j],".given.other.",covariate[i])]
