...
all_frem_plots <- function(postfremdir) {
  if(file.exists(file.path(postfremdir,"covdata.csv")) &&
    ...
    covdata <- read.csv(file.path(postfremdir,"covdata.csv"),header = T, as.is = T,check.names = F)
    pardata <- read.csv(file.path(postfremdir,"pardata.csv"),header = T, as.is = T,check.names = F)
    if(file.exists(file.path(postfremdir,"frem_ratio.csv"))) {
      inTable_frem <- read.csv(file.path(postfremdir,"frem_ratio.csv"),header = T, as.is = T,check.names = F)
    ...
    if(file.exists(file.path(postfremdir,"frem_id_ratios.csv"))) {
      frem_id <- read.csv(file.path(postfremdir,"frem_id_ratios.csv"), header = T, as.is = T,check.names = F)
    ...
    if(file.exists(file.path(postfremdir,"sd_coefficients_summary.csv"))) {
      sd_coef_summary <- read.csv(file.path(postfremdir,"sd_coefficients_summary.csv"), header = T, as.is = T,check.names = F)
